...
import { posicoes as posicoesMock, setores } from '@/lib/mock-data'
import type { Posicao } from '@/lib/mock-data'
...
function fmtMoeda(n: number, moeda: string) {
  const simbolo = moeda.split(' ')[1] ?? '€'
  return n.toLocaleString('pt-PT', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + ' ' + simbolo
}

/* ── Modal de adicionar posição ── */
function AdicionarModal({ onClose, onAdd }: {
  onClose: () => void
  onAdd: (p: Posicao) => void
}) {
  ...
  function confirmar() {
    ...
    onAdd({
      ...
      tipo:   selecionado.tipo as Posicao['tipo'],
      unid:   q,
      pm:     p,
      atual:  p, // sem cotação real, valor atual = PM
    })
    onClose()
  }

  const podeConfirmar = selecionado && quantidade && pm
      ...
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-sm
        ...
        <div className="flex items-center justify-between px-5 pt-2 pb-4 border-b border-stone-100">
          <div className="flex items-center gap-3">
            {step === 'detalhe' && (
              <button onClick={voltar} className="text-[13px] text-stone-500">
                ← Voltar
              </button>
            )}
        ...
        <div className="px-5 pt-4 space-y-4">
          ...
          {step === 'detalhe' && selecionado && (
            <>
              ...
              {/* Preço médio + moeda na mesma linha */}
              <div>
                <label className="block text-[12px] font-medium text-stone-500 mb-1.5">
                  ...
                  <div className="relative">
                    ...
                    {moedaOpen && (
                      <div className="absolute right-0 top-full mt-1 bg-white border border-stone-200
                        ...
                        {MOEDAS.map(m => (
                          <button key={m}
                            onClick={() => { setMoeda(m); setMoedaOpen(false) }}
              ...
              {quantidade && pm && (
                <div className="bg-stone-50 border border-stone-200 rounded-xl px-4 py-3
                  ...
                  <p className="text-[15px] font-semibold text-stone-900">
                    {fmtMoeda(parseFloat(quantidade || '0') * parseFloat(pm || '0'), moeda)}
                  </p>
                </div>
              )}
              ...
              <button
                ...
                className="w-full bg-brand-400 text-white font-medium text-[15px]
                  py-[13px] rounded-xl disabled:opacity-40 active:scale-[0.98]
                  transition-all">
...
export default function Portfolio() {
  ...
  const [posicoes,    setPosicoes]   = useState<Posicao[]>(posicoesMock)
  ...
  const melhor = [...posicoes].sort((a,b) =>
    ((b.atual - b.pm) / b.pm) - ((a.atual - a.pm) / a.pm))[0]
  const pior   = [...posicoes].sort((a,b) =>
    ((a.atual - a.pm) / a.pm) - ((b.atual - b.pm) / b.pm))[0]

  function adicionarPosicao(nova: Posicao) {
    const existente = posicoes.findIndex(p => p.ticker === nova.ticker)
    if (existente >= 0) {
      // Se já existe, faz média ponderada do PM
      const anterior = posicoes[existente]
      const totalUnid = anterior.unid + nova.unid
      const pmMedio   = (anterior.pm * anterior.unid + nova.pm * nova.unid) / totalUnid
      const novas = [...posicoes]
      novas[existente] = { ...anterior, unid: totalUnid, pm: pmMedio }
      setPosicoes(novas)
    } else {
      ...
      <div className="px-4 pt-4 space-y-3">
        ...
        {filtradas.length === 0 ? (
          <div className="bg-white rounded-2xl border border-stone-200 p-8 text-center">
            <p className="text-[14px] text-stone-500 mb-1">Nenhuma posição aqui.</p>
            <button
              onClick={() => setModalAberto(true)}
          ...
          <div className="bg-white rounded-2xl border border-stone-200 overflow-hidden">
            {filtradas.map((p, i) => {
              const ganho      = (p.atual - p.pm) / p.pm * 100
              const valorTotal = p.atual * p.unid
              return (
                <div key={p.ticker + i}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] text-stone-400">{p.unid} un · PM {fmt(p.pm)}</p>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <p className="text-[13px] font-medium text-stone-900">{fmt(valorTotal)}</p>
                    <p className={`text-[11px] font-medium ${ganho >= 0 ? 'text-brand-600' : 'text-red-500'}`}>
          ...
          <div className="bg-white rounded-2xl border border-stone-200 p-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="bg-stone-50 rounded-xl p-3">
                <p className="text-[11px] text-stone-500 mb-1">Melhor posição</p>
                <p className="text-[14px] font-semibold text-brand-600">
                  {melhor.ticker} +{(((melhor.atual - melhor.pm) / melhor.pm) * 100).toFixed(1)}%
                </p>
              </div>
              <div className="bg-stone-50 rounded-xl p-3">
                <p className="text-[11px] text-stone-500 mb-1">Pior posição</p>
                <p className="text-[14px] font-semibold text-red-500">
                  {pior.ticker} {(((pior.atual - pior.pm) / pior.pm) * 100).toFixed(1)}%
                </p>
              </div>
            </div>
          </div>
