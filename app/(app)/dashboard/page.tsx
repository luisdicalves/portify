  ...
  useEffect(() => {
    async function load() {
      ...
      const { data } = await supabase
        .from('perfis').select('nome, apelido').eq('id', session.user.id).single()
      const nome    = data?.nome    ?? session.user.user_metadata?.nome    ?? ''
      const apelido = data?.apelido ?? session.user.user_metadata?.apelido ?? ''
      ...
      <div className="px-4 pt-4 space-y-3">
        <div className="bg-white rounded-2xl border border-stone-200 p-4">
          ...
          <div className="flex items-center gap-1 text-brand-600 text-[13px] mb-4">
            <TrendingUp size={14} strokeWidth={2} /><span>+1.240 € este mês (+4,5%)</span>
          </div>
          <div className="flex gap-1 mb-3">
            {TIME_TABS.map(t => (
              <button key={t} className={`flex-1 text-[11px] py-[5px] rounded-md transition-colors
                ${t==='3M' ? 'bg-brand-50 text-brand-800 font-medium' : 'text-stone-500 hover:bg-stone-50'}`}>{t}</button>
            ))}
          </div>
          <ResponsiveContainer width="100%" height={90}>
            <AreaChart data={evolucao} margin={{ top:4, right:0, left:0, bottom:0 }}>
              ...
              <XAxis dataKey="mes" hide/><YAxis domain={['dataMin - 1000','dataMax + 1000']} hide/>
              <Tooltip contentStyle={{ fontSize:11, borderRadius:8, border:'0.5px solid #E8E6E0' }} formatter={(v:number)=>[fmt(v),'']}/>
